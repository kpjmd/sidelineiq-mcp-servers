...
export function registerWebTools(server: McpServer): void {
  ...
  server.tool(
    "web_thread_update_dates",
    "Persist resolved injury/surgery dates and provenance onto an injury thread (the injury_entities row). Called by the pre-OTM date-resolution loop, the post-publish projection patch, and MD manual date entry. Every field is optional and COALESCE'd against the current value — omit a field to leave it untouched. needs_date_review auto-clears when confidence is not 'unknown' unless set explicitly. canonical_post_id is the one exception to that rule: it is fill-if-null, not overwrite. It backfills threads created before their first post existed, so passing it when the thread already has a canonical is a no-op — an established thread is never repointed at a later follow-up post. When injury_date CHANGES and the thread already carries an otm_projection, projected_return_date is recomputed arithmetically from the STORED week window against the new anchor, and an 'otm_projection_reanchored' row is written to audit_log. The week window itself is never touched — it is a clinical judgement about the injury, not about the calendar — and OTM is not re-run. Pass an explicit otm_projection to override the recomputation. updated_by names the writer for the audit actor (any value other than 'system' records an 'md' actor); mirrors web_thread_close's closed_by.",
    {
      entity_id: z.string().uuid(),
